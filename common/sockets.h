...
struct CROSEServerConfig
{
    ...
    unsigned int        ServerID;                 //id from this server
    unsigned int        ParentID;                 //id from the server to connect
    char*       ServerName;               //Server name
    unsigned int        ServerType;               //0 login - 1 server(char) - 2 channel(world)
    BYTE        Connection;
	...
	unsigned int SaveTime;                   // Autosave time
    unsigned int EXP_RATE;
    unsigned int DROP_RATE;
    unsigned int ZULY_RATE;
    unsigned int AUTOSAVE;
    unsigned int SAVETIME;
    unsigned int MapDelay;
    unsigned int VisualDelay;
    unsigned int WorldDelay;
    unsigned int AIWatch;                    //LMA: AIP, Set an AI script number to watch for. Debug only
    char* WELCOME_MSG;
    unsigned int DROP_TYPE;
    int MaxStat;
    int Partygap;
    int FairyMode;
    int FairyStay;
