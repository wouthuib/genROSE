...
void CServerSocket::ServerLoop( )
{
	...
	//20070623, 221000
	unsigned int time_last_ping=clock();
	unsigned int time_last_client_ping=clock();
	unsigned int delay_ping=3600000;
	unsigned int delay_client_ping=5000;    //We do this each 5 seconds.
	//LMA END
	...
	do
	{
        ...
        //MySQL Ping
        //20070623, 221000
        unsigned int etime = (unsigned int)round((clock( ) - time_last_ping));
        if(etime>=delay_ping)
        {
        ...
        etime = (unsigned int)round((clock( ) - time_last_client_ping));
...
void CServerSocket::PingClients()
{
    for(unsigned int i=0;i<ClientList.size( );i++)
	{
        ...
        unsigned int etime = (unsigned int)round((clock( ) - client->CxTime));
        if(etime>30000)
        {
