        ...
        for(UINT i=0;i<GServer->MapList.Map.size();i++)
        {
            ...
            if (!only_npc||only_summon)
            {
                ...
                for(UINT j=0;j < map->PlayerList.size();j++)
                {
                    ...
                    if(player->IsDead( ))
                    {
                     ...
                     {
			if(difftime(time(NULL), player->lastSpawnUpdate) > player->playertime)
...

			{
                             ...
                             if(player->last_monstercid!=0)
                             {
                                ...
                                map->DeleteMonster( thismon );
                                */
                                player->playertime= time(NULL) + 10.0;
                             ...
                             else
                             {
 	                         ...
 	                         {
                                    player->lastSpawnUpdate = time(NULL);
                                    fPoint position;
		    ...
		    if (player->Status->Stance==DRIVING&&(player->last_fuel>0) && difftime(time(NULL),  player->last_fuel) > 60.0)
                    {
                      //We kill some fuel every now and then :)
                      player->TakeFuel();
                      player->last_fuel=time(NULL);
                    }

                    //LMA: mileage coupon checks.
                    double etime = difftime(time(NULL), player->timer_no_exp);

                    if(player->no_exp && (etime >= 120.0))
                    {
                      BEGINPACKET( pak, 0x702 );
                      ADDSTRING( pak, "[Mileage] Null Xp vanished.");
                      ADDBYTE( pak, 0 );
                      player->client->SendPacket(&pak);
                      player->timer_no_exp=time(NULL);
                      player->no_exp=false;
                    }

		    etime = difftime(time(NULL), player->timerxp);

                    if(player->bonusxp > 1 && (etime >= 180.0))
                    {
                      BEGINPACKET( pak, 0x702 );
                      ...
                      player->client->SendPacket(&pak);
                      player->bonusxp=1;
                      player->timerxp=time(NULL);
                      player->wait_validation=0;
                    }

		    etime = difftime(time(NULL), player->timerddrop);
                    if(player->bonusddrop > 1 && (etime >= 240.0))
                    {
                      BEGINPACKET( pak, 0x702 );
                      ...
                      player->client->SendPacket(&pak);
                      player->bonusddrop=1;
                      player->timerddrop=time(NULL);
                      player->wait_validation_ddrop=0;
                    }

		    etime = difftime(time(NULL), player->timerstatdrop);
                    if(player->bonusstatdrop > 1 && (etime >= 300.0))
                    {
                      BEGINPACKET( pak, 0x702 );
                      ...
                      player->timerstatdrop=time(NULL);
		    ...
		    //Log(MSG_INFO,"%s :: %d - Monster Update Position, Monster stay still = '%d' (1 = true)",__FILE__, __LINE__, monster->stay_still);
			...
			if(difftime(time(NULL), monster->thisnpc->AiTimer) > 6.0)
                        {
                            monster->thisnpc->AiTimer = time(NULL);
			...
			if(difftime(time(NULL), monster->lastAiUpdate) > monster->thisnpc->AiTimer)
                        {
            ...
            for(UINT j=0;j < map->NPCList.size();j++)
            {
                ...
                if(npc->thisnpc->AI != 0)
                {
                    ...
                    int delay = 60;    //each AIP 60 seconds.
                    //LMA: AIP Timer.
                    delay = npc->thisnpc->AiTimer;

                    if(npc->thisnpc->AiTimer==0)
                    {
                        npc->thisnpc->AiTimer=60;
		     ...
		     if(difftime(time(NULL), npc->lastAiUpdate) > 60.0)
                     {
                         ...
                         NPCmonster->Position->lastMoveTime = time(NULL);
                         NPCmonster->SpawnTime = time(NULL);
                         NPCmonster->lastSighCheck = time(NULL);
    ...
    while(GServer->ServerOnline)
    {
        ...
        for(UINT i=0;i<GServer->MapList.Map.size();i++)
        {
            ...
            for(UINT j=0;j<map->PlayerList.size();j++)
            {
                ...
                {
                    //time_t etime = time(NULL) - player->lastSaveTime;
                    if(difftime(time(NULL), player->lastSaveTime) > 60.0)
                    {
                        player->savedata( );
                        player->lastSaveTime = time(NULL);
                    }
                }
            }
        }
