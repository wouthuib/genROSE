...
bool CMap::TeleportPlayer( CPlayer* player, fPoint coordold, bool TelePassenger )
{
    ...
    player->Position->lastMoveTime = time(NULL);
...
void CMap::RespawnMonster( )
{
    ...
    for (UINT j = 0; j < MobGroupList.size(); j++)
    {
      ...
      //timer_t rtime = time(NULL) - thisgroup->lastRespawnTime;
      //timer_t rtime = difftime(time(NULL), thisgroup->lastRespawnTime);
      //if (rtime < thisgroup->respawntime*CLOCKS_PER_SEC || thisgroup->active >= thisgroup->limit)
      if(difftime(time(NULL), thisgroup->lastRespawnTime) > thisgroup->respawntime || thisgroup->active >= thisgroup->limit)
      {
          if(lma_debug)
          {
              Log(MSG_INFO,"Spawn %u:: No respawn (time %u < %u ? or limit %u>=%u ?)",thisgroup->id, difftime(time(NULL), thisgroup->lastRespawnTime), thisgroup->respawntime,thisgroup->active,thisgroup->limit);
          }
