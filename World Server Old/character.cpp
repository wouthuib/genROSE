...
//    Stats->ShieldDamage = 0;
    ...
    assert(Position);
    Position->Map = 0;
    Position->lastMoveTime = time(NULL);
    Position->saved = 0;
    Position->respawn = 0;
...
*/
	...
	//Log(MSG_INFO," Monster Walk, (%.2f:%.2f)->(%.2f:%.2f)",Position->current.x,Position->current.y,Position->destiny.x,Position->destiny.y);
	Log(MSG_INFO," Monster ntime = '%.2f', etime = '%.2f', distance = '%.2f'", ntime, etime, distance);
        //LMA: bad, that's bad...
	if (ntime <= etime || distance < 0.01)
    	...
    	{
		//Log(MSG_INFO," Monster Movement: X + %f, Y + %f", dx*(etime/ntime), dy*(etime/ntime));
