...
bool CMonster::OnEnemyOnSight( CPlayer* Enemy )
{
    ...
    double etime = difftime(time(NULL), lastSighCheck);
...
bool CMonster::SummonUpdate(CMonster* monster, CMap* map, UINT j)
{
    ...
    double etime = difftime(time(NULL), lastDegenTime);

    if( etime >= 4.0 && Stats->HP > 0 )
    {
