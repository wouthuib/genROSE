...
bool CPlayer::GetPlayerInfo( )
{
	...
	ADDBYTE( pak, 0 );
	client->SendPacket(&pak);
    sprintf(text,"HP: %lli/%lli | MP: %lli/%lli",Stats->HP,Stats->MaxHP,Stats->MP,Stats->MaxMP);
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
...
bool CPlayer::RefreshHPMP()
{
    ...
    time_t etime = difftime(time(NULL), lastShowTime);
    if ( etime > 5.0 )
        return true;
...
bool CPlayer::Regeneration()
{
	...
	double etimeHP = difftime(time(NULL), lastRegenTime_hp);
	...
	double etimeMP = difftime(time(NULL), lastRegenTime_mp);
...
bool CPlayer::PlayerHeal()
{
    ...
    double transtime = difftime(time(NULL), UsedItem->lastRegTime);
    if( UsedItem->usevalue !=0 && transtime >= 3.0 )
    {
        if( UsedItem->used < UsedItem->usevalue && Stats->HP > 0 )
        {
            ...
            UsedItem->used += value;
            UsedItem->lastRegTime = time(NULL);
        }
        else
        {
...
void CPlayer::TakeFuel(int add_fuel)
{
    ...
    conso_fuel*=250*(time(NULL)-last_fuel)/(60*1000*100);
...
bool CPlayer::CheckPortal()
{
    ...
    double etime = difftime(time(NULL), lastportalchecktime);
    if ( etime >= 2.0 )
    {
        lastportalchecktime = time(NULL);
...
bool CPlayer::CheckEvents()
{
   ...
   double etime = difftime(time(NULL), lastEventTime);
   double rtime = difftime(time(NULL), RefreshEventTimer);
