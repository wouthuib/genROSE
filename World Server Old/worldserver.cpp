...
void CWorldServer::OnClientDisconnect( CClientSocket* thisclient )
{
    ...
    if ( player->Fairy )
    {
        ...
        FairyList.at(player->FairyListIndex)->LastTime = time(NULL);
...
void CWorldServer::ServerLoop( )
{
	...
	do
	{
        ...
        //MySQL Ping
        //20070623, 221000
        UINT etime = difftime(time(NULL), time_last_ping );//(UINT)round((clock( ) - time_last_ping));
        if(etime >= delay_ping)
        {
            time_last_ping=time(NULL);
            Ping();
        }
        //LMA END
...
bool CWorldServer::OnServerReady( )
{
    ...
    */
	...
	//double loadtime = difftime(time(NULL), timer);
