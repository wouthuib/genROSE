...
bool CWorldServer::pakDoID( CPlayer* thisclient, CPacket* P )
{
	...
	thisclient->firstlogin=time;    //LMA for fairy
...
bool CWorldServer::pakMoveChar( CPlayer* thisclient, CPacket* P )
{
    ...
    thisclient->Position->lastMoveTime = time;
...
bool CWorldServer::pakDoDrop( CPlayer* thisclient, CPacket* P )
{
	...
	if (itemid == 0)
		...
		Log(MSG_INFO,"Item dropid=0, therefore zuly drop. File %u, Line %u" __FILE__, __LINE__);
		if ( amount<1 )
		{
		    return true;
		}
...
bool CWorldServer::pakPickDrop( CPlayer* thisclient, CPacket* P )
{
	...
	ADDWORD    ( pak, thisdrop->clientid );
    CPlayer* dropowner = 0;
    if( (thisdrop->owner==0 || thisdrop->owner==thisclient->CharInfo->charid || time - thisdrop->droptime>=30 ) || ( thisclient->Party->party!=NULL && thisclient->Party->party == thisdrop->thisparty ))
    {
...
bool CWorldServer::pakNormalChat( CPlayer* thisclient, CPacket* P )
{
    ...
    else
    {
        ...
        thisclient->next_chat=time + 1000;   //Next second check.
...
bool CWorldServer::pakShout ( CPlayer* thisclient, CPacket* P )
{
    ...
    thisclient->next_shout = time + 19000;   //Can shout at this time.
...
bool CWorldServer::pakWhisper ( CPlayer* thisclient, CPacket* P )
{
    ...
    else
    {
        ...
        thisclient->next_whisper= time + 1000;   //Next second check.
...
bool CWorldServer::pakChatUnion ( CPlayer* thisclient, CPacket* P )
{
    ...
    else
    {
        ...
        thisclient->next_chat_union= time + 1000;   //Next second check.
...
bool CWorldServer::pakChatTrade ( CPlayer* thisclient, CPacket* P )
{
    ...
    else
    {
        ...
        thisclient->next_chat_trade= time + 1000;   //Next second check.
