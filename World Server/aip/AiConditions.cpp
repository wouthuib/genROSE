...
AICOND(002)
{
	...
	*/
    ...
    if(data->btIsAllied == 1)
    {
        ...
        for(unsigned int j=0; j< map->MonsterList.size();j++)
        {
    ...
    else
    {
        ...
        for(unsigned int j=0; j< map->PlayerList.size();j++)
        {
...
AICOND(024)
{
    ...
    if(data->btDate != 0)
    {
    	if(sTIME->tm_mday != data->btDate)
    		return AI_FAILURE;
    }

    word wMin = ((sTIME->tm_hour * 60) + sTIME->tm_min);
	...
	struct tm * TimeTo;
	time ( &rawtime );
  	TimeFrom = localtime ( &rawtime );
        TimeTo = localtime ( &rawtime );
	...
	mktime( TimeTo );
        ...
        if(wFrom==19)
        {
            ...
            TimeFrom->tm_min=GServer->GemQuestForce;
            TimeTo->tm_min=TimeFrom->tm_min+2;
        }
        else
        {
            //quest LTB shout (20:00:00 to 20:01:00)
            TimeFrom->tm_min=GServer->GemQuestForce+4;
            TimeTo->tm_min=TimeFrom->tm_min+1;
        }
	...
	mktime( TimeFrom );
        ...
        wFrom=((TimeFrom->tm_hour * 60) + TimeFrom->tm_min);
        wTo=((TimeTo->tm_hour * 60) + TimeTo->tm_min);
    }
	...
	time_t rawtime;
        ...
        for (int k=0;k<12;k++)
        {
            if(k==0)
            {
                for (int j=0;j<6;j++)
                {
                    TimeFrom->tm_min=28+delay[j];
                    TimeTo->tm_hour=TimeFrom->tm_hour;
                    TimeTo->tm_min=TimeFrom->tm_min+1;

		    mktime( TimeFrom );
        	    mktime( TimeTo );

                    from = ((TimeFrom->tm_hour * 60) + TimeFrom->tm_min);
                    to = ((TimeTo->tm_hour * 60) + TimeTo->tm_min);
			...
			TimeFrom->tm_hour = 0;
        		...
        		mktime( TimeTo );
                        ...
                        TimeFrom->tm_min=GServer->UWForceFrom+delay[j];
                        TimeTo->tm_min=TimeFrom->tm_min+1;
    ...
    LogDebug("AIC24 time h(%i>=%i>=%i)? m(%i>=%i>=%i)?",temph1,sTIME->tm_hour,temph,tempm1,sTIME->tm_min,tempm);
...
AICOND(025)
{
    ...
    if(data->btWeekDay != sTIME->tm_wday) return AI_FAILURE;
    word wMin = ((sTIME->tm_hour * 60) + sTIME->tm_min);
    word wFrom = (data->btHour1 * 60) + data->btMin1;
    word wTo = (data->btHour2 * 60) + data->btMin2;
...
AICOND(028)
{
    ...
    unsigned int tempval = thisMonster->AIVar[data->nVarIDX];
...
AICOND(030)
{
    ...
    //double diff_time = difftime(time(NULL), thisMonster->SpawnTime);
    long int diff_time = static_cast<long int>(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());

    if(diff_time >= data->Timer * 1000)
    {
        LogDebug("AIC030:: timer ok %u>=%u" ,diff_time,data->Timer * 1000);
        return AI_SUCCESS;
    }
    else
    {
        LogDebug("AIC030:: timer NOT ok %u<%u" ,diff_time,data->Timer * 1000);
    }
