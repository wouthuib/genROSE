...
AIACT(003)
{
    ...
    //Log(MSG_WARNING,"%s :: %d - Monster Update Position, Monster stay still = '%d' (1 = true)",__FILE__, __LINE__, monster->stay_still);
    ...
    entity->Position->lastMoveTime = time;
...
AIACT(008)
{
	...
	GETAIACTDATA(008);
        CMonster* monster = reinterpret_cast<CMonster*>(entity);
	//Log(MSG_WARNING,"%s :: %d - Monster Update Position, Monster stay still = 'false'",__FILE__, __LINE__);
	...
	monster->thisnpc->stance = data->cSpeed;
	monster->SetStats(false);
    monster->Position->lastMoveTime = time;
    ClearBattle( entity->Battle );// this also seems to clear the attackers battle??
    //entity->Battle->atktype = 0;
    BEGINPACKET( pak, 0x797 );
	ADDWORD    ( pak, entity->clientid );
