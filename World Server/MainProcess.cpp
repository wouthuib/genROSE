...
{
    long int time_now = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
    bool ok_cont=false;
    UINT loopcount=0;
    long int time_skill=0;
            ...
            if (!only_npc||only_summon)
            {
                ...
                for(UINT j=0;j<map->PlayerList.size();j++)
                {
                    ...
                    if(player->IsDead( ))
                    {
                     ...
                     {
                         if(player->playertime < (UINT)GServer->round((time_now - player->lastSpawnUpdate)))
                         {
                             ...
                             else
                             {
                                 ...
                                 {
                                     player->lastSpawnUpdate=time_now;
                                    fPoint position;
                    ...
                    //Fuel handling.
                    if (player->Status->Stance==DRIVING&&(player->last_fuel>0)&&(time_now - player->last_fuel>60000))
                    {
                      //We kill some fuel every now and then :)
                      player->TakeFuel();
                      player->last_fuel=time_now;
                    }

                    //LMA: mileage coupon checks.
                    long int etime = time_now;
                ...
                // Monster update //------------------------
		time_now = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
                ...
                for(UINT j=0;j < map->MonsterList.size();j++)
                {
                    ...
                    {
        		//thisnpc->lastAiUpdate = time_now;
                        if(1000 < (UINT)GServer->round(time_now - monster->lastAiUpdate) )
                        {
                            //LogDebug("DoAIP mainprocess monster loop %i",monster->thisnpc->AI);
                            monster->hitcount = 0;
                            monster->DoAi(monster->thisnpc->AI, 0);
                            monster->lastAiUpdate=time_now;
                        }
                    ...
                    if(monster->IsOnBattle( ))
                    {
                        ...
                        if(monster->thisnpc->AiTimer < (UINT)GServer->round(time_now - monster->lastAiUpdate) )
                        {
            ...
            for(UINT j=0;j<map->NPCList.size();j++)
            {
                ...
                if(npc->thisnpc->AI != 0)
                {
                    ...
                    //LMA END
                     ...
                     //if(is_time_ok)
                     if(delay < (UINT)GServer->round(time_now - npc->lastAiUpdate) )
                     {
                        ...
                        NPCmonster->Position->destiny = npc->pos;
                         ...
                         NPCmonster->Position->lastMoveTime = time_now;
                         NPCmonster->SpawnTime = time_now;
                         NPCmonster->lastSighCheck = time_now;
                         NPCmonster->lastLifeUpdate = time_now;
    ...
    while(GServer->ServerOnline)
    {
        ...
        for(UINT i=0;i<GServer->MapList.Map.size();i++)
        {
            ...
            for(UINT j=0;j<map->PlayerList.size();j++)
            {
                ...
                {
                    long int etime = time_now - player->lastSaveTime;
		    //printf("Savetime = %u, etime = %u, File/Line = %s, %s\n", GServer->Config.SAVETIME, etime, __FILE__, __LINE__ );
                    if( etime >= GServer->Config.SAVETIME )
                    {
                        player->savedata( );
                        player->lastSaveTime = time_now;
                    }
                }
            }
        }
