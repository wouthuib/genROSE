...
void CWorldServer::OnClientDisconnect( CClientSocket* thisclient )
{
    ...
    if ( player->Fairy )
    {
        ...
        FairyList.at(player->FairyListIndex)->LastTime = time;
...
{
    long int timer = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
    long int time = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
    ...
    */
	...
	float loadtime = (float)( time - timer ) / 1000;
