...
bool CPlayer::GetPlayerInfo( )
{
	...
	ADDBYTE( pak, 0 );
	client->SendPacket(&pak);
    sprintf(text,"HP: %lli/%lli | MP: %lli/%llu",Stats->HP,Stats->MaxHP,Stats->MP,Stats->MaxMP);
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
...
bool CPlayer::RefreshHPMP()
{
    ...
    long int etime = time - lastShowTime;
    if ( etime < 1000 )
        return true;
...
bool CPlayer::Regeneration()
{
    ...
    if (Stats->MaxHP==Stats->HP)
       lastRegenTime_hp = time;

    if (Stats->MaxMP==Stats->MP)
      lastRegenTime_mp = time;
    ...
    if ((Stats->MaxHP==Stats->HP)&&(Stats->MaxMP==Stats->MP))
     ...
     int TimeDiff = nb_sec_stance * 1000; // * CLOCKS_PER_SEC;

    //LMA: HP
    if (Stats->HP < Stats->MaxHP)
    {
        long int etimeHP = time - lastRegenTime_hp;

	//Log(MSG_INFO,"etimeHP = %u, lastRegenTime_hp = %u, TimeDiff = %u, Stats->HP = %u", etimeHP, lastRegenTime_hp, TimeDiff, Stats->HP);
    ...
    if(Stats->MP<Stats->MaxMP)
    {
        long int etimeMP = time - lastRegenTime_mp;
...
{
    long int time = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
    long int transtime = time - UsedItem->lastRegTime;
	if( UsedItem->usevalue!=0 && transtime >= 0.3 * 1000 )
    {
...
void CPlayer::TakeFuel(int add_fuel)
{
    ...
    conso_fuel*=250*(time-last_fuel)/(60*1000*100);
...
bool CPlayer::CheckPortal()
{
    long int time = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
    //CPlayer* thisclient = GServer->GetClientByCharName( CharInfo->charname );
    long int etime = time - lastportalchecktime;

    if ( etime >= 2 * 1000 )
    {
        lastportalchecktime = time;
...
bool CPlayer::CheckEvents()
{
   //CPlayer* thisclient = GServer->GetClientByCharName( CharInfo->charname );
   long int time = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
   long int etime = time - lastEventTime;
   long int rtime = time - RefreshEventTimer;
   if ( etime >= 2 * 1000 )
   {
       lastEventTime = time;
...
int CPlayer::ExecuteQuestTrigger(dword hash,bool send_packet, UINT index)
{
    ...
    if (trigger == NULL)
    {
        Log(MSG_WARNING,"EXTP, questid %u not found. TriggerList Size %u",hash, GServer->TriggerList.size());
