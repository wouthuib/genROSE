...
bool CWorldServer::pakGMCommand( CPlayer* thisclient, CPacket* P )
{
   ...
   if(strcmp(command, "addquest")==0)
        ...
        for(int i=0;i<10;i++)
            {
                if (thisclient->quest.quests[i].QuestID != 0) continue;
                    ...
                    thisclient->quest.quests[i].StartTime = time;
   ...
   else if (strcmp(command, "b")==0)
    {
        ...
        {
            thisclient->CharInfo->LastGlobal = time;
            char outputmsg[200];
   ...
   else if (strcmp(command, "bonusxp")==0)
	{
        ...
        else
        {
           ...
           otherclient->wait_validation=0;
           otherclient->timerxp = time + time_min *60;
           otherclient->Saved=false;
           if (to_save!=0)
   ...
   else if (strcmp(command, "cart")==0)     // get all castlegear parts
    {
        ...
        BEGINPACKET( pak, 0);
        {
            ...
            {
                unsigned int itemtype=atoi(row[1]);
                unsigned int itemnum=atoi(row[0]);
...
//		ADDSTRING( pak, "DELETESPAWN" );
        ...
        for(unsigned int i=0;i<map->MonsterList.size();i++)
        {
    ...
    else if(strcmp(command, "testmon")==0)
    {
        ...
        {
            unsigned int monster_clientid=atoi(tmp);
            tmp = strtok(NULL, " ");
    ...
    else if(strcmp(command, "face")==0)
    {
        if(Config.Command_Face > thisclient->Session->accesslevel)
	       return true;
        if ((tmp = strtok(NULL, " ")) == NULL) return true; unsigned int face=atoi(tmp);
   ...
   else if (strcmp(command, "give2")==0)
    {
        if(Config.Command_Item > thisclient->Session->accesslevel)
           return true;
        unsigned int itemrefine, itemstats, itemls, itemsocket;
        if ((tmp = strtok(NULL, " "))==NULL) return true; char* name=tmp;
        if ((tmp = strtok(NULL, " "))==NULL) return true; unsigned int itemid =atoi(tmp);
        if ((tmp = strtok(NULL, " "))==NULL) return true; unsigned int itemtype =atoi(tmp);
        if ((tmp = strtok(NULL, " "))==NULL) return true; unsigned int itemamount =atoi(tmp);
        if ((tmp = strtok(NULL, " "))==NULL)
            itemrefine =0;
        else
   ...
   else if(strcmp(command, "hair")==0)
    {
        if(Config.Command_Hair > thisclient->Session->accesslevel)
	       return true;
        if ((tmp = strtok(NULL, " ")) == NULL) return true; unsigned int hair=atoi(tmp);
...
//        if (i > 0 && i%10 == 0) {
   ...
   else if(strcmp(command, "mystat")==0)    // mystat - by PurpleYouko
    {
        if ((tmp = strtok(NULL, " "))==NULL)
        {
         ...
         {
             //LMA: pvp team.
             sprintf ( buffer, "My team is %li, pvp status is %i", thisclient->pvp_id,thisclient->pvp_status);
             SendPM(thisclient, buffer);
         }
   ...
   else if(strcmp(command, "mystat2")==0)    // mystat2 - by PurpleYouko - from osprose to test
    {
         ...
         {
             char buffer2[200];
             sprintf ( buffer2, "My current HP is %lli", thisclient->Stats->HP );
             SendPM(thisclient, buffer2);
         }
         else if(strcmp(tmp, "mp")==0)
         {
             char buffer2[200];
             sprintf ( buffer2, "My current MP is %lli", thisclient->Stats->MP );
             SendPM(thisclient, buffer2);
         }
         else if(strcmp(tmp, "maxhp")==0)
         {
             char buffer2[200];
             sprintf ( buffer2, "My Maximum HP is %lli", thisclient->GetMaxHP());
             SendPM(thisclient, buffer2);
         }
...
//         }
   ...
   if(Config.Command_Set > thisclient->Session->accesslevel)
   ...
   else if (strcmp(command, "shutdown")==0)
	{
        ...
        if ((tmp = strtok(NULL, " "))==NULL) return true;
            unsigned long minutes =atoi(tmp);
        char text[200];
        sprintf( text, "Server will shutdown in %lui minutes, Please logout NOW to be sure your information is saved correctly.",
           minutes );
        BEGINPACKET( pak, 0x702 );
   ...
   else if(strcmp(command, "spawndetail")==0)
    {
        ...
        else if(strcmp(command, "spawnmonsters")==0)
        {
            ...
            thisclient->lastSpawnUpdate=time;
   ...
   else if(strcmp(command, "who2")==0)
    {
        ...
        sprintf(line0, "There are currently %lui players connected!", ((ClientList.size()-1)-hiddenam));
        SendPM(thisclient, line0 );
...
bool CWorldServer::pakGMItem( CPlayer* thisclient, unsigned int itemid, unsigned int itemtype, unsigned int itemamount, unsigned int itemrefine, unsigned int itemdura, unsigned int itemstats, unsigned int itemsocket )
{
    CItem item;
...
bool CWorldServer::pakGMItemQuest( CPlayer* thisclient, unsigned int itemid, unsigned int itemtype, unsigned int itemamount, unsigned int itemrefine, unsigned int itemls, unsigned int itemstats, unsigned int itemsocket, char buffer2[200] )
{
    CItem item;
    item.count            = itemamount;
    item.durability        = 40;
...
bool CWorldServer::pakGMEventType(CPlayer* thisclient, int npctype, int dialog, long int type)
{
    ...
    sprintf( buffer, "Event %li for NPC: %i , Dialog: %i (if dialog has changed, type here to force refresh)", type, npctype, dialog);
...
bool CWorldServer::ReloadMobSpawn( CPlayer* thisclient, int id )
{
	...
	thisspawn->lastRespawnTime=time;
    return true;
}
...
bool CWorldServer::pakGMFairyto(CPlayer* thisclient, char* name, int mode)
{
    ...
    if(!otherclient->Fairy && mode == 1)
    {
          ...
          GServer->FairyList.at(FairyIndex)->LastTime = time;
...
// Give Item to Player
bool CWorldServer::pakGMItemtoplayer(CPlayer* thisclient, char* name , unsigned int itemid, unsigned int itemtype, unsigned int itemamount, unsigned int itemrefine, unsigned int itemls, unsigned int itemstats, unsigned int itemsocket)
{
    CItem item;
    item.count            = itemamount;
    item.durability        = 40;
...
bool CWorldServer::GMShowTargetInfo( CPlayer* thisclient )
{
	...
	ADDBYTE( pak, 0 );
    thisclient->client->SendPacket( &pak );
    sprintf( buffer, "Target Attack Speed: %ui", monster->Stats->Attack_Speed );
    RESETPACKET( pak, 0x784 );
    ADDSTRING  ( pak, "[SYS]TargetInfo" );
    ADDBYTE    ( pak, 0 );
    ADDSTRING  ( pak, buffer );
	...
	ADDBYTE( pak, 0 );
    thisclient->client->SendPacket( &pak );
    sprintf( buffer, "Target HP/MAXHP: %lli/%llui", monster->Stats->HP, monster->Stats->MaxHP );
    RESETPACKET( pak, 0x784 );
    ADDSTRING  ( pak, "[SYS]TargetInfo" );
    ADDBYTE    ( pak, 0 );
    ADDSTRING  ( pak, buffer );
...
{
    for(unsigned int j = 0; j < MapList.Index[thisclient->Position->Map]->MonsterList.size(); j++)
    {
...
{
    unsigned int map=thisclient->Position->Map;
...
bool CWorldServer::pakGMSpawnDetail(CPlayer* thisclient, unsigned int id, unsigned int map)
{
    if (map>maxZone)
    {
    ...
    for(unsigned int j = 0; j < mymap->MonsterList.size(); j++)
    {
...
bool CWorldServer::pakGMSpawnForceRefresh(CPlayer* thisclient, unsigned int id, unsigned int map)
{
    if (map>maxZone)
    {
        Log(MSG_INFO,"Map %u does not exist",map);
    ...
    //searching now where are the monsters using this spawnid.
    //doing a mutext to be sure we're alone touching this one.
    vector<unsigned int> monstertokill;
    pthread_mutex_lock( &mymap->MonsterMutex );
    for(unsigned int j = 0; j < mymap->MonsterList.size(); j++)
    {
        CMonster* thismon = mymap->MonsterList.at(j);
    ...
    for(unsigned int j=0;j<monstertokill.size();j++)
    {
