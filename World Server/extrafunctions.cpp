...
unsigned CWorldServer::GetNewClientID( )
{
    ...
    {
		if (ClientIDList[i]!=0 && time - ClientIDList[i]>10)
        {
			ClientIDList[i] = 0;
...
CPlayer* CWorldServer::GetClientByUserName( char *username )
{
    for(UINT i=0;i<ClientList.size();i++)
    {
		...
		if (strcasecmp(thisclient->Session->username,username)==0)
            return thisclient;
	}
	return NULL;
}
...
CPlayer* CWorldServer::GetClientByCharNameCI( char* name  )
{
    ...
    {
        CPlayer* thisclient = (CPlayer*) ClientList.at(i)->player;
        if (strncasecmp(thisclient->CharInfo->charname,name, 16)==0)
            return thisclient;
	}
	return NULL;
}
        ...
        case 321://Time Coupon - New Way to check TC
        {
              ...
              switch (UseList.Index[useitem->itemnum]->useeffect[0])
              {
                    case 138: //LMA: Null exp.
                    {
                        ...
                        thisclient->timer_no_exp = time + 60000 * (UseList.Index[useitem->itemnum]->useeffect[1]*10);     //Effective Time Duration
                    ...
                    case 96://Special Shop Coupon, Shop A/B/C, Moldie, snowman, ....
                    {
                        ...
                        thisclient->Shop->mil_shop_time = time + 60000 *(UseList.Index[useitem->itemnum]->useeffect[1]*10);     //Effective Time Duration
                        BEGINPACKET( pak, 0x702 );
                    ...
                    case 133://Special Exp Time Coupon, Adventurer, Trainee, ...
                    {
                        if (thisclient->timerxp == 0)
                        {
                            ...
                            if(useitem->itemnum==200||useitem->itemnum==948)
                            {
                                ...
                                thisclient->timerxp= time + 60000 * 60;  //1 hour
                            ...
                            else if (useitem->itemnum==199)
                            {
                                ...
                                //valid until logout
                                thisclient->once=true;
                                thisclient->timerxp = time + 30000 *60 ;  //1 hour
                                //Log(MSG_INFO,"Bonus XP to %i",thisclient->bonusxp);
                                BEGINPACKET( pak, 0x702 );
                            ...
                            else if(useitem->itemnum==203)
                            {
                                ...
                                thisclient->timerxp = time + 86400000 * 3;
                            ...
                            else
                            {
                                ...
                                //thisclient->timerxp=clock()+86400*1000;
                                thisclient->timerxp = time + 86400000;
                                thisclient->once=false;
                                //Log(MSG_INFO,"Bonus XP to %i",thisclient->bonusxp);
                                BEGINPACKET( pak, 0x702 );
                    ...
                    case 134://Medal of Fortune
                    {
                        if (thisclient->timerddrop == 0)
                        {
                            ...
                            thisclient->timerddrop = time + 60000 *(UseList.Index[useitem->itemnum]->useeffect[1]*10);  //Effective Time Duration
                    ...
                    case 135://Medal of Excellence
                    {
                        if (thisclient->timerstatdrop == 0)
                        {
                            ...
                            thisclient->timerstatdrop = time + 60000 *(UseList.Index[useitem->itemnum]->useeffect[1]*10);  //Effective time duration
                    ...
                    case 136://Medal of Retrieval
                    {
                        if (thisclient->timergraydrop == 0)
                        {
                            ...
                            thisclient->timergraydrop= time + 60000 *(UseList.Index[useitem->itemnum]->useeffect[1]*10);  //Effective time duration
