		...
		case 0:
		{
            ...
            for(DWORD i = 0; i < 10; i++)
            {
                ...
                if(data->iQuestSN>=2851&&data->iQuestSN<=2858)
                {
                    ...
                    unsigned int zaward=5000*client->uw_kills;
                    client->uw_kills=0;
                    if (zaward>0)
                    {
...
QUESTREWD(001)
{
  ...
  if( client->questdebug )
    ...
    {
        unsigned int nb_items=curQuest->AddItem(&tmpItem, data->btOp);
...
QUESTREWD(007)
{
	...
	{
        for(unsigned int i=0;i<client->Party->party->Members.size( );i++)
        {
...
QUESTREWD(023){
  ...
  if( client->questdebug )
    ...
    for(unsigned int i=0;i<GServer->MapList.Map.size();i++)
    {
