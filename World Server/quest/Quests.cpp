...
void CWorldServer::ReadQSD(char* path, dword index)
{
	...
	dword RecordCount = fh->Get<dword>();
		...
		//printf("Quests.cpp::108 - file length = %u, loopnr = %u, loopmax =%u\n",len,j,RecordCount);
		//std::cin.get();
		...
		//printf("Quests.cpp::150 \t\t Function %i / %i (%s / %lu)\n", j+1,RecordCount,trigger->TriggerName,MakeStrHash(trigger->TriggerName));
                if(trigger->ConditionCount > 0 && len > 0) // update to avoid long loops
                {
                    if(lma_export)
                    {
...
void CWorldServer::LoadQuestData()
{
	...
	for(dword i = 1; i < stbQuest->Rows(); i++)
	{
		...
		//LMA: Patch for Pegasus server data files:
	        ...
	        {
	            string temps=buffer;
	            temps="3DDATA/"+temps.substr(7);
               	    strcpy ((char*)&buffer,temps.c_str());
               	    buffer[temps.size()]=0;
	        }

		//printf("Quests.cpp::332 - %s\n", (char*)&buffer);
	    	GServer->ReadQSD((char*)&buffer,i);
	    }
	}
...
void CWorldServer::ExportQSDData(byte* dataorg, int size,int opcode)
{
    ...
    if(opcode==1)
    {
        ...
        {
            unsigned long address = i * sizeof(STR_QUEST_DATA);
            address += (intptr_t)dataorg;
            address += 4;
