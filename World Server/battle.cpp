        ...
        case NORMAL_ATTACK://normal attack
        {
            ...
            */
            ...
            if(IsTargetReached( Enemy ) && CanAttack( ))
            {
                if(IsMonster())
                {
                   //Log(MSG_INFO,"monster Doing a normal attack %d, %s", __LINE__, __FILE__);
                }
                else
                {
                   //Log(MSG_INFO,"player Doing a normal attack %d, %s", __LINE__, __FILE__);
                }
			    ...
			    //Log(MSG_INFO,"Monster %i is attacked, Timer = '%u'",monster->clientid, GServer->round(time - monster->nextAi_attacked));
                            //LMA: Doing is_attacked from time to time, not at each attack...
                            if(0 <= GServer->round(time - monster->nextAi_attacked) )
                            {
                                monster->nextAi_attacked = time;
                                long int temp = monster->nextAi_attacked;
                               // Log(MSG_INFO,"Monster %i is attacked, doing his AI 3, HP: %I64i",monster->clientid,monster->Stats->HP);
                                monster->DoAi(monster->thisnpc->AI, 3);
   ...
   // Log(MSG_INFO,"pak 799 : clientid %i, Enemy->clientid %i",clientid,Enemy->clientid);
    ...
    if(Enemy->IsDead())
    {
        ...
        if(!Enemy->IsSummon( ) && !Enemy->IsPlayer( ))
        {
		    ...
		    Log(MSG_WARNING,"No drops found for monster %u. File = %u, Line = %u", Enemy->clientid, __FILE__, __LINE__);
    ...
    time = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
    Battle->lastAtkTime = time;
    ReduceABC( );
...
bool CCharacter::SkillAttack( CCharacter* Enemy, CSkills* skill )
{
    ...
    if(Battle->castTime==0)
    {
        ...
        Battle->castTime = time;
        return true;
    }
    else
    {
        long int etime = time - Battle->castTime;
...
void CCharacter::QuestKill(dword die_quest)
{
    ...
    long int mytime = time;
    long int oldest_time = mytime;
    int oldest=0;
    ...
    plkiller->arr_questid[oldest].die_time=mytime+(1000 / 2);
