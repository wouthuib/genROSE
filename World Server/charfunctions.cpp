...
void CCharacter::StartAction( CCharacter* Target, BYTE action, UINT skillid, bool restart, CCharacter* receiver )
{
    ...
    //Log(MSG_INFO,"Start action %i",action);
    switch(action)
    {
        case NORMAL_ATTACK:
        {
            ...
            Position->lastMoveTime = time;
	    //Log(MSG_INFO,"move speed %u, base speed %u, lastmovetime = %u",Stats->Move_Speed,Stats->Base_Speed, Position->lastMoveTime);
        }
        break;
        case SKILL_ATTACK:
        case SKILL_BUFF:
...
bool CCharacter::CanAttack( ) // updated by Core
{
    ...
    long int etime = time - Battle->lastAtkTime;
    ...
    //Log(MSG_INFO,"Check CanAttack weapontype %i stun %i etime %u ATTK_SPEED_MODIF %u aspeed %u",weapontype, Status->Stun ,etime, GServer->ATTK_SPEED_MODIF, Stats->Attack_Speed);
    //Log(MSG_INFO,"etime %u < stat->Attack_Speed %u, Postion->Map %i", etime, 100 * (GServer->ATTK_SPEED_MODIF*4/3) / Stats->Attack_Speed, Position->Map );
    ...
    if ( weapontype == BOW || weapontype == GUN || weapontype == LAUNCHER || weapontype == CROSSBOW)
    {
       ...
       if( (etime < 1000 * GServer->ATTK_SPEED_MODIF / Stats->Attack_Speed) || Status->Stun != 0xff )
       {
            //LMA: logs
           if(Position->Map==8)
           {
    ...
    //Log(MSG_INFO,"CanAttack true weapontype %i stun %i etime %u ATTK_SPEED_MODIF %u aspeed %u",weapontype,Status->Stun,etime,GServer->ATTK_SPEED_MODIF,Stats->Attack_Speed);
    return true;
}
...
void CCharacter::RefreshBuff( )
{
    ...
    for( UINT i=0;i<30;i++)
    {
        ...
        long int etime = time - MagicStatus[i].BuffTime;
        ...
        else if ( ((MagicStatus[i].Status >= 7 && MagicStatus[i].Status <= 11) || MagicStatus[i].Status == 89) && etime > 1 * 1000) //Do poison dmg every 1.5 seconds
        {
            ...
            if(IsDead())
            {
             ...
             if( IsDead())
             {
			    ...
			    Log(MSG_WARNING,"Cannot find drops for monster file %u, line %u", __FILE__, __LINE__);
        ...
        else if ( MagicStatus[i].Status == 58 || MagicStatus[i].Status == 61 || MagicStatus[i].Status == 71 || MagicStatus[i].Status >= 77 && MagicStatus[i].Status <= 80 && etime > 1* 1000) //Do flame dmg every 1.5 seconds
        {
             Stats->HP -= MagicStatus[i].Status;
             MagicStatus[i].BuffTime += 1 * 1000;
             MagicStatus[i].Duration -= 1;
             printf("did %i flame dmg to the player, still %i seconds and %lli HP remain \n", MagicStatus[i].Status, MagicStatus[i].Duration, Stats->HP);
