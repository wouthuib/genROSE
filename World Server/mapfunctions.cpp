...
bool CMap::TeleportPlayer( CPlayer* player, fPoint coordold, bool TelePassenger )
{
    ...
    player->Position->lastMoveTime = time;
...
void CMap::UpdateTime( )
{
    long int time = static_cast<long int>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
    if(id==0) return;
    UINT etime = (UINT)round((time - LastUpdate)/1000);
    if(etime<10)return;
...
void CMap::RespawnMonster( )
{
    ...
    for (UINT j = 0; j < MobGroupList.size(); j++)
    {
      ...
      long int rtime = time - thisgroup->lastRespawnTime;
      if (rtime < thisgroup->respawntime * 1000 || thisgroup->active >= thisgroup->limit)
      {
          if(lma_debug)
          {
              Log(MSG_INFO,"Spawn %u:: No respawn (time %u < %u ? or limit %u>=%u ?)",thisgroup->id,rtime,thisgroup->respawntime * 1000,thisgroup->active,thisgroup->limit);
          }
      ...
      if(thisgroup->lastKills>=thisgroup->basicMobs.at(last_group)->real_amount)
      {
          if(!thisgroup->group_ready)
          {
              ...
              thisgroup->lastRespawnTime = time;
