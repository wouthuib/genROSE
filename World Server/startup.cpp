...
bool CWorldServer::InitDefaultValues()
{
    ...
    ClassList=new vector<unsigned int>*[ListClass.rowcount];
    ...
    for(unsigned int i=0;i<maxBreak;i++)
    {
    ...
    for(int i=0;i<10;i++)
    {
        ...
        for(unsigned int j=0;j<EquipList[i].max;j++)
    ...
    for(unsigned int i=0;i<NaturalList.max;i++)
    {
    ...
    for(unsigned int i=0;i<PatList.max;i++)
    {
    ...
    for(unsigned int i=0;i<SellList.max;i++)
    {
    ...
    for(unsigned int i=0;i<MapList.max;i++)
    {
    ...
    for(unsigned int i=0;i<UseList.max;i++)
    {
    ...
    for(unsigned int i=0;i<ProductList.max;i++)
    {
    ...
    for(unsigned int i=0;i<JemList.max;i++)
    {
...
bool CWorldServer::LoadQuestItemData( )
{
    ...
    for (unsigned int i = 0; i<STB_ITEM[12].rowcount; i++)
    {
...
bool CWorldServer::LoadNPCData( )
{
    ...
    for (int i = 0; i<STB_NPC.rowcount; i++)
    {
        CNPCData* newnpc = new (nothrow) CNPCData;
        if(newnpc==NULL)
        {
...
bool CWorldServer::LoadMobGroupsSpecial()
{
  ...
  while (row = mysql_fetch_row(result))
  {
    ...
    thisgroup->lastRespawnTime = duration_cast<duration<unsigned int>>(high_resolution_clock::now() * 1000);
    thisgroup->active = 0;
    thisgroup->basicKills = 0;
    thisgroup->curTac = 0;
    thisgroup->curBasic = 0;
...
bool CWorldServer::LoadJobReq( )
{
    ...
    {
        vector<unsigned int>* myvector=new vector<unsigned int>;
        myvector->clear();
    ...
    for(unsigned int i=0;i<BreakData.rowcount;i++)
    {
        ...
        switch (choice)
        {
                ...
                case 2:
                {
                   ...
                   //We read the 20 items.
                   int nb_rewards=0;
                   unsigned int rewtype=0;
                   unsigned int rewnum=0;
                   bool isok=true;
                   newchest->probmax = 0;
