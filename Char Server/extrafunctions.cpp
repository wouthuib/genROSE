...
CCharClient* CCharServer::GetClientByUserID( unsigned int userid )
{
...
CCharClient* CCharServer::GetClientByName( char *name )
{
    for(unsigned int i=0;i<ClientList.size( );i++)
	{
        ...
        if(strcasecmp(client->charname, name)==0)
        {
            return client;
        }
	}
...
{
    for(unsigned int i=0;i<ClanList.size();i++)
    {
...
bool CCharServer::SendToClanMembers( int clanid,CPacket* pak )
{
    ...
    if(thisclan==0)
        return false;
    for(unsigned int i=0;i<thisclan->ClanMembers.size();i++)
    {
...
int CCharServer::GetNbUserID( unsigned int userid )
{
    int nb_accounts=0;
    for(unsigned int i=0;i<ClientList.size( );i++)
	{
...
bool CCharServer::DiscoAllAvatars(unsigned int userid )
{
